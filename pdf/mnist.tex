...
\enter\indent
That's all. See \fig{1.1} for an example initial state and an example final state. We also supply the animation \href{fig/fig\_1\_1.gif}{\inline{fig\_1\_1.gif}} which illustrates how a calculation proceed.
...
These restrictions are needed since the original equations \refe{2.5}, ..., \refe{2.8} are not of the symmetric forms\footnote{Reversing "$\leq$" we get not "$\geq$" but "$>$". This gives rise to an asymmetry.}.

\subsubsection{OR Gate}

An {\it OR gate} is a gate which returns \inline{true} if at least one of the inputs is \inline{true}. The corresponding truth table is shown as \tab{2.3}.
...
Let's consider a gate called an {\it XOR gate}, or an {\it Exclusive OR gate}. It returns \inline{true} if only one of the two inputs is \inline{true}. See \tab{2.4} for the truth table.
...
Here's a good news: this limitation can be overwhelmed by using a {\it multilayer perceptron}. It is simply a collection of usual perceptrons arrayed in layers. The network in \fig{2.1} we showed above is an example of a multilayer perceptron. Since OR returns \inline{false} only for $(0, 0)$ and NAND returns \inline{false} only for $(1, 1)$, by taking AND of their outputs, we get the final result \inline{true} only for $(0,1)$ or $(1, 0)$. Thus the multilayer perceptron shown in \fig{2.3} behaves as an XOR gate.
...
As described in \sect{2.1}, the output $y$ of a perceptron follows \refe{2.3} and \refe{2.4}. These equations can be rewritten as
\begineq
y &=& h (S) \la{3.1} \\
...
The operation $IW + \bmb$ is called a {\it affine transformation}. Since $L$ is of the length $M$, this itself is used as the input to the next layer $l_3$. Therefore, one forward propagation is processed just by repeatedly calculating \refe{3.12}.
...
\begin{algorithm}
    ...
    \For {$p$ in $P$}
        \State $p \gets p + \eps$
        \State $E1 \gets (\text{the loss function})$ \Comment{$f(x+\eps)$}
        \State $p \gets p - 2\eps$
        \State $E2 \gets (\text{the loss function})$ \Comment{$f(x-\eps)$}
        \State $p \gets p + \eps$
...
Actually we can even calculate the derivative with respect to the variables in a layer at the same time. Just aligning \refe{3.20} to \refe{3.22} we get
\begineq
...
\begin{__theorem}
Let $g$ be the final output of a network, $I$ an input to an instance of \inline{SigmoidLayer} class and $Y$ the corresponding output of the instance of \inline{SigmoidLayer} class. Then the formula below is satisfied.
\begineq
...
\begin{__theorem}
Let $g$ be the final output of a network, $I$ an input to an instance of \inline{ReluLayer} class and $Y$ the corresponding output of the instance of \inline{ReluLayer} class. Then
\begineq
...
We implement a network whose structure is shown in \fig{3.6}. We set the number of the nodes in the input layer to $28 \times 28 = 784$ and that of the output layer to $10$ since there are $10$ kinds of digits. We use the classes whose structure is shown in \fig{4.2} and handle all the layers though pointers of the type \inline{Layer *} stored in \inline{layer\_} array except the instance of \inline{SoftmaxCrossEntropyLayer} which is exceptionally pointed by \inline{last\_layer\_} of the type \inline{LastLayer *}.
...
\enter
{\bf Further Reading}\\
This paper is mainly based on the book \href{https://www.oreilly.co.jp/books/9784873117584/}{Koki Saitoh {\it "Deep Learning from Scratch"} (O'Reilly Japan, 2016)}. However note the book relatively relies on intuitive explanations and has little math. That's why we decided to write this paper.
